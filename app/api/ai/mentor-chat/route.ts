...
export async function POST(request: NextRequest) {
  try {
    const { message, userId, username, careerPath, conversationHistory, fileUrl, fileName, fileType } = await request.json()
    ...
    if (!apiKey) {
      ...
      return NextResponse.json({ 
        response: "I'm sorry, but I'm not fully configured right now. Please check back later! 😊" 
      })
    }
...
Respond as Noni, keeping in mind this student's background and our conversation history. Be helpful, encouraging, and focused on their learning journey.`
    ...
    if (fileUrl && fileName) {
      messageWithFile = `${message}\n\n[Student shared a file: ${fileName}${fileType ? ` (${fileType})` : ''}]`
    }
